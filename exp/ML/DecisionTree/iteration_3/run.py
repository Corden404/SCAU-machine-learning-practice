...
def pruning_path_alphas(x: pd.DataFrame, y: pd.Series) -> list[float]:
    path = DecisionTreeClassifier(random_state=RANDOM_SEED).cost_complexity_pruning_path(x, y)
    alphas = sorted({float(alpha) for alpha in path.ccp_alphas if float(alpha) >= 0.0})
    if len(alphas) > 1:
        # The last alpha usually collapses the tree to the root; keep the path but drop
        # that degenerate candidate from model selection.
        alphas = alphas[:-1]
    ...
    ).to_csv(OUTPUT_DIR / "cost_complexity_pruning_path.csv", index=False, encoding="utf-8-sig")
    return alphas
...
def grid_search() -> tuple[dict[str, Any], dict[str, Any]]:
    x, y = load_ionosphere()
    candidate_alphas = pruning_path_alphas(x, y)
    ...
    search = GridSearchCV(
        ...
        param_grid={"ccp_alpha": candidate_alphas},
        scoring=scoring,
        refit="macro_f1",
        cv=splitter,
        n_jobs=-1,
    ...
    best_info = {
        "candidate_ccp_alpha_count": len(candidate_alphas),
        "candidate_ccp_alpha_min": float(min(candidate_alphas)),
        "candidate_ccp_alpha_max": float(max(candidate_alphas)),
...
if __name__ == "__main__":
    ...
    run_experiment(
        ...
        experiment_note=(
            "DecisionTree with cost-complexity pruning. "
            "Only ccp_alpha is tuned from the pruning path by 5-fold CV macro F1. "
